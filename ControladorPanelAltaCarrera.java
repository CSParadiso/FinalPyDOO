...
public class ControladorPanelAltaCarrera {
	...
	void botonCrearMateriaPresionado(String nombreCarrera, String nombreMateria, int cuatrimestreMateria, int notaPromocionar,
		...
		if (carrera != null && !carrera.getNombresMaterias().contains(nombreMateria) &&
			...
			if (materiaPromocionable) // acá puede ir polimorfismo
				carrera.nuevaMateriaPromocionable(nombreMateria, cuatrimestreMateria, notaPromocionar, aprobarCursada, aprobarFinal, opcional);
			else
			...
			if (!hayCupoOpcionales(carrera, opcional))
				PanelAltaCarrera.campoNombreMateriaNueva.setText("¡No se pueden definir más materias opcionales!");
			else
	...
	//This model (SpinnerListModel) inherits a ChangeListener. The ChangeListeners are notified whenever the model's value or list properties changes.
	void spinnerSeleccionarCarreraRecorrido(String nombreCarrera) { 
		if (nombreCarrera == "Seleccione una carrera" | Facultad.buscarCarrera(nombreCarrera) == null) 
			PanelAltaCarrera.spinnerListaMaterias.setList(new ArrayList<String>(){{add("Seleccione una materia existente");}});
		else {
			carrera = Facultad.buscarCarrera(nombreCarrera);
			if (carrera.getNombresMaterias().isEmpty()) 
				PanelAltaCarrera.spinnerListaMaterias.setList(new ArrayList<String>() {{add("Materia sin carreras");}});
			else 
				PanelAltaCarrera.spinnerListaMaterias.setList(carrera.getNombresMaterias());
		}
	...
	void spinnerSeleccionarMateriaRecorrido(String nombreMateria) {
		materia = carrera.buscarMateria(nombreMateria);
		//System.out.println("Nombre que llega desde la materia buscada: " + materia.getNombre());
		//System.out.println("Nombre que llega al método seleccionarMateriaRecorrido: " + nombreMateria);
	...
	void botonAgregarCorrelativasPresionado(List<String> materiasCorrelativas) {
		...
		while (iterador.hasNext()) {
			...
			if (!materia.getNombresCorrelativas().contains(nombreCorrelativa)) 
				materia.añadirCorrelativa(carrera.buscarMateria(nombreCorrelativa));
		}
		mostrarInfoMateriaActual();
	};
	...
	private void mostrarInfoMateriaActual() {
		if (materia != null) {		
			...
			PanelAltaCarrera.campoInfoNotaAprobarCursada.setText(((Integer)(materia.getNotaParaAprobarCursada())).toString());
