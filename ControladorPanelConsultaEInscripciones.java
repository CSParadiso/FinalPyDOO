...
public class ControladorPanelConsultaEInscripciones {
	...
	void actualizarListaEstudiantes() {
		...
		while (iterador.hasNext()) {
			...
			PanelConsultaEInscripcion.modeloTablaEstudiantes.addRow(new String[] {((Integer)estudiante.getLegajo()).toString(), 
	...
	void visualizarDatosEstudiante() {
		...
		PanelConsultaEInscripcion.campoLegajo.setText(buscarEstudiante() == null ? "" : ((Integer)buscarEstudiante().getLegajo()).toString());
		PanelConsultaEInscripcion.campoRecibido.setText(buscarEstudiante() == null ? "" : buscarEstudiante().getCarrerasRecibido().toString());
		PanelConsultaEInscripcion.campoCursando.setText(buscarEstudiante() == null ? "" : buscarEstudiante().getCarreras().toString());
		PanelConsultaEInscripcion.campoCursandoMateria.setText(buscarEstudiante() == null ? "" : buscarEstudiante().getCursando().toString());
	...
	private Estudiante buscarEstudiante() {
		try{
			Object valorLegajo = PanelConsultaEInscripcion.tablaEstudiantes.getValueAt
					(PanelConsultaEInscripcion.tablaEstudiantes.getSelectedRow(), 0); 
